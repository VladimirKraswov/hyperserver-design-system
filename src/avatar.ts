...
] as const satisfies readonly AgentAvatarPalette[]

const sizeMap: Readonly<Record<Exclude<AgentAvatarSize, number>, number>> = {
  xs: 24,
  sm: 32,
  md: 44,
  lg: 60,
...
export const hashAgentAvatarSeed = (seed: string): number => {
  let hash = 2166136261
...
export const createAgentAvatarProfile = (seed: string): AgentAvatarProfile => {
  ...
  return {
    ...
    variant: (hash >>> 3) % 6,
    tilt: ((hash >>> 11) % 7) - 3,
    antenna: (["left", "center", "right"] as const)[antennaIndex] ?? "center",
  }
}

export const resolveAgentAvatarSize = (size: AgentAvatarSize = "md"): number =>
  typeof size === "number" ? Math.max(20, Math.round(size)) : sizeMap[size]
